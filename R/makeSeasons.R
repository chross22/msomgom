...
makeSeasons <- function(begYEAR, endYEAR, ssn_beg, ssn_end) {
  ...
  names(season) <- c("Y", "M_BEG", "D_BEG", "M_END", "D_END",
                     "JDAY_BEG", "JDAY_END", "SSN_NO", "SSN_GRPD_NO")

  ctr <- 1
  for (i in seq_along(uy)) {
    for (j in seq_len(num_ssn)) {
      season[ctr, 1] <- uy[i]         # year
      season[ctr, 2] <- ssn_beg[j, 1] # month the season begins
      season[ctr, 3] <- ssn_beg[j, 2] # day the season begins
      season[ctr, 4] <- ssn_end[j, 1] # month the season ends
      season[ctr, 5] <- ssn_end[j, 2] # day the season ends
      season[ctr, 8] <- ctr           # absolute season number
      season[ctr, 9] <- j             # season number within the year

      ctr <- ctr + 1
    }
  }

  # Yearday for the beginning and end of each season, into columns 6 and 7.
  season[, 6] <- as.numeric(format(
    as.Date(paste(season[, 1], season[, 2], season[, 3], sep = "-"),
            "%Y-%m-%d"), "%j"))
  season[, 7] <- as.numeric(format(
    as.Date(paste(season[, 1], season[, 4], season[, 5], sep = "-"),
            "%Y-%m-%d"), "%j"))
